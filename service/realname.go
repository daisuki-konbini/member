...
func (s *Service) realnameInfo(c context.Context, mid int64) (info *model.RealnameCacheInfo, err error) {
	...
	if cacheFlag {
		s.cache.Save(func() {
			if theErr := s.mbDao.SetRealnameInfo(context.Background(), mid, info); theErr != nil {
				log.Error("%+v", theErr)
			}
		})
	}
...
func (s *Service) RealnameApply(c context.Context, mid int64, captureCode int, realname string, cardType int8, cardCode string, country int16, handIMGToken, frontIMGToken, backIMGToken string) (err error) {
	...
	// }
	s.cache.Save(func() {
		if theErr := s.mbDao.DeleteRealnameCaptureCode(context.Background(), mid); theErr != nil {
			log.Error("%+v", err)
		}
		if theErr := s.mbDao.DeleteRealnameInfo(context.Background(), mid); theErr != nil {
			log.Error("%+v", err)
		}
	})
	return
...
func (s *Service) checkCapture(c context.Context, mid int64, capture int) (err error) {
	...
	if capture != serverCode {
		s.cache.Save(func() {
			if errTimes < 0 {
				if theErr := s.mbDao.SetRealnameCaptureErrTimes(context.Background(), mid, 0); theErr != nil {
					log.Error("%+v", theErr)
					return
				}
			}
			if theErr := s.mbDao.IncreaseRealnameCaptureErrTimes(context.Background(), mid); theErr != nil {
				log.Error("%+v", theErr)
				return
			}
		})
...
func (s *Service) RealnameStrippedInfo(ctx context.Context, mid int64) (*v1.RealnameStrippedInfoReply, error) {
	info, err := s.realnameInfo(ctx, mid)
	if err != nil {
		return nil, err
	}
	stripped := &v1.RealnameStrippedInfoReply{
		Mid:      info.MID,
...
func (s *Service) MidByRealnameCard(ctx context.Context, arg *v1.MidByRealnameCardsReq) (*v1.MidByRealnameCardReply, error) {
	...
	return &v1.MidByRealnameCardReply{CodeToMid: codeToMid}, nil
}

...
const (
	_shard = 100
	// detail
	_setTags = "INSERT INTO user_detail_%02d(mid,tags) VALUES (?,?) ON DUPLICATE KEY UPDATE tags=?"
	// base
	...
	// official
	_setOfficialDoc = `INSERT INTO user_official_doc (mid,name,state,role,title,description,extra,submit_source) VALUES (?,?,?,?,?,?,?,?) 
	ON DUPLICATE KEY UPDATE name=VALUES(name), state=VALUES(state), role=VALUES(role), title=VALUES(title), description=VALUES(description), extra=VALUES(extra), submit_source=VALUES(submit_source)`
...
// SetTags set tags.
func (d *Dao) SetTags(c context.Context, mid int64, tags string) (err error) {
	if _, err = d.db.Exec(c, fmt.Sprintf(_setTags, hit(mid)), mid, tags, tags); err != nil {
		err = errors.Wrapf(err, "dao set detail mid(%d)", mid)
	}
	return
}
...
func (d *Dao) SetOfficialDoc(c context.Context, od *model.OfficialDoc) (err error) {
	_, err = d.db.Exec(c, _setOfficialDoc, od.Mid, od.Name, model.OfficialStateWait, od.Role,
		od.Title, od.Desc, od.OfficialExtra.String(), od.SubmitSource)
	if err != nil {
